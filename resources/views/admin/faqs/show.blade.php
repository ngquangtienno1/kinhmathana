                                ...
                                @foreach($images as $image)
                                    <div class="position-relative">
                                        <img src="{{ asset('storage/' . $image) }}" alt="FAQ Image"
                                            class="img-thumbnail" style="max-width: 200px; height: 200px; object-fit: cover;">
                                    </div>
                                @endforeach
                            @endif
                        </div>
                ...
                <div class="col-12">
                    <div class="d-flex gap-2">
                        ...
                        <form action="{{ route('admin.faqs.destroy', $faq->id) }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="btn btn-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa FAQ này?')">

        ...
        <div class="d-flex flex-wrap gap-3">
            <div class="search-box">
                <form class="position-relative" action="{{ route('admin.permissions.index') }}" method="GET">
                    ...
                    @if(request('search'))
                        <a href="{{ route('admin.permissions.index') }}" class="btn btn-link position-absolute end-0 top-50 translate-middle-y p-0 me-2">
                    ...
                    <tr>
                        <th class="white-space-nowrap fs-9 align-middle ps-0" style="width:20px;">
                            <div class="form-check mb-0 fs-8">
                                <input class="form-check-input" id="checkbox-bulk-permissions-select" type="checkbox" data-bulk-select='{"body":"permissions-table-body"}' />
                            </div>
                        </th>
                ...
                <tbody class="list" id="permissions-table-body">
                    ...
                    <tr class="position-static">
                        <td class="fs-9 align-middle">
                            <div class="form-check mb-0 fs-8">
                                <input class="form-check-input" type="checkbox" data-bulk-select-row='{"name":"{{ $permission->name }}","slug":"{{ $permission->slug }}"}' />
                            </div>
                        </td>
                        ...
                        <td class="align-middle white-space-nowrap text-end pe-0 ps-4 btn-reveal-trigger">
                            <div class="btn-reveal-trigger position-static">
                                ...
                                <div class="dropdown-menu dropdown-menu-end py-2">
                                    <a class="dropdown-item" href="{{ route('admin.permissions.edit', $permission) }}">Sửa</a>
                                    <div class="dropdown-divider"></div>
                                    <form action="{{ route('admin.permissions.destroy', $permission) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" class="dropdown-item text-danger" onclick="return confirm('Bạn có chắc chắn muốn xóa quyền này?')">Xóa</button>
                                    </form>
                                </div>
                            </div>
                        </td>
        ...
        <div class="row align-items-center justify-content-between py-2 pe-0 fs-9">
            <div class="col-auto d-flex">
                <p class="mb-0 d-none d-sm-block me-3 fw-semibold text-body">
                    Hiển thị {{ $permissions->firstItem() ?? 0 }} đến {{ $permissions->lastItem() ?? 0 }} của {{ $permissions->total() }} kết quả
                </p>
            </div>
            <div class="col-auto d-flex">

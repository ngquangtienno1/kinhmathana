                                        ...
                                        <div class="qodef-e-inner">
                                            <div class="qodef-woo-product-image">
                                                ...
                                                <div class="qodef-woo-product-hover">
                                                    <a href="{{ route('client.cart.add', $product->id) }}" class="qodef-woo-product-icon qodef-add-to-cart" data-bs-toggle="tooltip" title="Thêm vào giỏ hàng">
                                                        <i class="fas fa-shopping-cart"></i>
                                                    </a>
                                                    <a href="#" class="qodef-woo-product-icon qodef-add-to-wishlist" data-bs-toggle="tooltip" title="Thêm vào yêu thích">
                                                        <i class="fas fa-heart"></i>
                                                    </a>
                                                    <a href="{{ route('client.products.show', $product->slug) }}" class="qodef-woo-product-icon qodef-quick-view" data-bs-toggle="tooltip" title="Xem nhanh">
                                            ...
                                            <div class="qodef-woo-product-content">
                                                ...
                                                <span class="price">
                                                    <span class="woocommerce-Price-amount amount">
                                                        <bdi><span class="woocommerce-Price-currencySymbol">$</span>{{ number_format($product->minimum_price, 2) }}</bdi>
                                                    </span>
                                                </span>
                                            </div>
                                            <a href="{{ route('client.products.show', $product->slug) }}"
                    ...
                    <div class="qodef-grid-item qodef-page-sidebar-section qodef-col--3 qodef-col-pull--9">
                        <aside id="qodef-page-sidebar" role="complementary">
                                ...
                                <ul class="product-categories">
                                    @foreach ($categories as $category)
                                        <li class="cat-item cat-item-{{ $category->id }}"><a
                                                href="{{ route('client.products.index', ['category_id' => $category->id]) }}">{{ $category->name }}</a></li>
                                    @endforeach
                                </ul>
                            </div>
                                ...
                                <form method="get" action="{{ route('client.products.index') }}">
                                    <div class="price_slider_wrapper">
                                        ...
                                        <div class="price_slider_amount" data-step="10">
                                            ...
                                            <input type="text" id="min_price" name="min_price" value="{{ request('min_price', 110) }}"
                                                data-min="110" placeholder="Min price" />
                                            <label class="screen-reader-text" for="max_price">Max price</label>
                                            <input type="text" id="max_price" name="max_price" value="{{ request('max_price', 520) }}"
                                ...
                                <ul class="woocommerce-widget-layered-nav-list">
                                    @foreach ($colors as $color)
                                        <li class="woocommerce-widget-layered-nav-list__item wc-layered-nav-term">
                                            <a rel="nofollow" href="{{ route('client.products.index', ['colors' => [$color->id]]) }}">{{ $color->name }}</a>
                                            <span class="count">({{ $products->getCollection()->flatMap->variations->where('color_id', $color->id)->count() }})</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                            <div class="widget woocommerce widget_layered_nav woocommerce-widget-layered-nav"
                                ...
                                <ul class="woocommerce-widget-layered-nav-list">
                                    @php
                                        $sizes = $products->getCollection()->flatMap->variations->pluck('size')->unique();
                                    @endphp
                                    @foreach ($sizes as $size)
                                        <li class="woocommerce-widget-layered-nav-list__item wc-layered-nav-term">
                                            <a rel="nofollow" href="{{ route('client.products.index', ['size' => $size->name ?? $size]) }}">{{ $size->name ?? $size }}</a>
                                            <span class="count">({{ $products->getCollection()->flatMap->variations->where('size_id', $size->id ?? null)->count() }})</span>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
                            <div class="widget widget_block" data-area="shop-sidebar">
                                <div data-block-name="woocommerce/product-search" data-label=""
                                    ...
                                    <form role="search" method="get"
                                        action="{{ route('client.products.index') }}"><label
                            ...
                            <div class="widget widget_block widget_media_image" data-area="shop-sidebar">
                                <figure class="wp-block-image size-large"><a href="#"><img

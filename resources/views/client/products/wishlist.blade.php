                                ...
                                <table class="qwfw-m-items shop_table">
                                    ...
                                    <tbody class="qwfw-m-items-content">
                                        @forelse ($wishlists as $item)
                                            <tr class="qwfw-m-items-content-row qwfw-e qwfw--simple" data-item-id="{{ $item->product->id }}">
                                                <td class="qwfw-e-item product-remove">
                                                    <div class="qwfw-e-item-inner">
                                                        <form method="POST" action="{{ route('client.wishlist.remove') }}">
                                                            ...
                                                            <button type="submit" class="qwfw-e-remove-button qwfw-spinner-item" aria-label="Remove this item" style="background:none;border:none;padding:0;margin:0;line-height:1;">
                                                                <span class="qwfw-e-remove-button-icon">
                                                                    <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18">
                                                                        <line x1="1" y1="1" x2="17" y2="17" stroke="red" stroke-width="2"/>
                                                                        <line x1="17" y1="1" x2="1" y2="17" stroke="red" stroke-width="2"/>
                                                                    </svg>
                                                                </span>
                                                            </button>
                                                        </form>
                                                ...
                                                <td class="qwfw-e-item product-thumbnail">
                                                    <div class="qwfw-e-item-inner">
                                                        <a href="{{ route('client.products.show', $item->product->slug) }}">
                                                            <img src="{{ $item->product->images->first()->url ?? '/images/no-image.jpg' }}" alt="{{ $item->product->name }}" width="80">
                                                        </a>
                                                    </div>
                                                </td>
                                                <td class="qwfw-e-item product-name">
                                                    <div class="qwfw-e-item-inner">
                                                        <a class="qwfw-e-item-name" href="{{ route('client.products.show', $item->product->slug) }}">
                                                            {{ $item->product->name }}
                                                        </a>
                                                    </div>
                                                </td>
                                                ...
                                                <td class="qwfw-e-item product-stock-status">
                                                    <div class="qwfw-e-item-inner">
                                                        <span class="qwfw-e-item-stock {{ $item->product->stock_quantity > 0 ? 'qwfw--in-stock' : 'qwfw--out-of-stock' }}">
                                                            {{ $item->product->stock_quantity > 0 ? 'Còn hàng' : 'Hết hàng' }}
                                                        </span>
                                                    </div>
                                                </td>
                                                <td class="qwfw-e-item product-add-to-cart">
                                                    <div class="qwfw-e-item-inner">
                                                        <form method="POST" action="{{ route('client.cart.add') }}">
                                                            @csrf
                                                            <input type="hidden" name="product_id" value="{{ $item->product->id }}">
                                                            <button type="submit" class="single_add_to_cart_button button alt" style="min-width:180px;min-height:48px;font-size:15px;letter-spacing:1px;white-space:nowrap;text-transform:uppercase;border:1.5px solid #222;background:#fff;color:#222;font-weight:500;box-shadow:none;outline:none;transition:background 0.2s, color 0.2s;">
                                                                Thêm giỏ hàng
                                                            </button>
                                                        </form>
                                                    </div>

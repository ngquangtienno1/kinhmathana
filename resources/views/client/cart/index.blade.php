              ...
              <table class="table fs-9 mb-0 border-top border-translucent">
                <thead>
                  <tr>
                    ...
                    <th class="sort white-space-nowrap align-middle" scope="col" style="min-width:250px;">SẢN PHẨM</th>
                    <th class="sort align-middle" scope="col" style="width:80px;">MÀU</th>
                    <th class="sort align-middle" scope="col" style="width:150px;">KÍCH THƯỚC</th>
                    <th class="sort align-middle text-end" scope="col" style="width:150px;">GIÁ</th>
                    <th class="sort align-middle ps-5" scope="col" style="width:120px;">SỐ LƯỢNG</th>
                    <th class="sort align-middle text-end" scope="col" style="width:150px;">TỔNG</th>
                ...
                <tbody class="list" id="cart-table-body">
                  ...
                  <tr class="cart-table-row btn-reveal-trigger">
                    <td class="align-middle white-space-nowrap py-0">
                      <a class="d-block border border-translucent rounded-2" href="#">
                        <img src="{{ $item->variation->product->images->first() ? asset('storage/'.$item->variation->product->images->first()->image_path) : '/assets/img/products/1.png' }}" alt="" width="53">
                      </a>
                    </td>
                    ...
                    <td class="quantity align-middle fs-8 ps-5">
                      <div class="input-group input-group-sm flex-nowrap" data-quantity="data-quantity">
                        <button class="btn btn-sm px-2 quantity-btn" data-type="minus" data-id="{{ $item->id }}">-</button>
                        <input class="form-control-plaintext text-center input-spin-none quantity-input"
                               type="number" min="1" value="{{ (int)$item->quantity }}"
                               data-id="{{ $item->id }}"
                               aria-label="Số lượng"
                               style="width: 40px; background: transparent; border: none; box-shadow: none; outline: none;" />
                        <button class="btn btn-sm px-2 quantity-btn" data-type="plus" data-id="{{ $item->id }}">+</button>
                      </div>
                    </td>
                    ...
                    <td class="align-middle white-space-nowrap text-end pe-0 ps-3">
                      <form action="{{ route('client.cart.remove', $item->id) }}" method="POST" onsubmit="return confirm('Xóa sản phẩm này khỏi giỏ hàng?')" style="display: inline;">
                        @csrf @method('DELETE')
                        <button type="submit" class="btn btn-sm text-body-tertiary text-opacity-85 text-body-tertiary-hover me-2">
                  ...
                  @endforeach
                  <tr class="cart-table-row btn-reveal-trigger">
                    <td class="text-body-emphasis fw-semibold ps-0 fs-8" colspan="6">Tổng tiền hàng :</td>
          ...
          <div class="card">
            <div class="card-body">
              <div class="d-flex flex-between-center mb-3">
                <h3 class="card-title mb-0">Tóm tắt</h3><a class="btn btn-link p-0" href="{{ route('client.cart.index') }}">Chỉnh sửa giỏ hàng</a>
              </div>
              ...
              <div>
                ...
                <div class="d-flex justify-content-between">
                  ...
                  <p class="text-danger fw-semibold summary-discount">-{{ number_format(0, 0, ',', '.') }}₫</p>
                </div>
              ...
              <div class="input-group mb-3 position-relative" style="z-index: 10;">
                  <input class="form-control" type="text" placeholder="Mã giảm giá" id="voucherInput" autocomplete="off">
                  <button class="btn btn-phoenix-primary px-5 apply-voucher-btn">Áp dụng</button>
                  <div id="voucherDropdown" class="dropdown-menu w-100" style="max-height: 200px; overflow-y: auto;">
                      @foreach($promotions as $promotion)
                          <button class="dropdown-item" type="button"
                              data-code="{{ $promotion->code }}">
                              <b>{{ $promotion->code }}</b> - {{ $promotion->name }}
                              <div class="small text-body-tertiary">{{ $promotion->description }}</div>
                          </button>
                      @endforeach
                  </div>
              </div>
              ...
              <a href="{{ route('client.cart.checkout.form') }}" class="btn btn-primary w-100">
                ...
                <svg class="svg-inline--fa fa-chevron-right ms-1 fs-10" aria-hidden="true" focusable="false" data-prefix="fas" data-icon="chevron-right" role="img" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 320 512" data-fa-i2svg="">
    ...
    applyBtn.addEventListener('click', function() {
        ...
        .then(data => {
            if (data.success) {
                ...
                discountElement.textContent = '-' + Math.abs(discount).toLocaleString('vi-VN') + '₫';

        ...
        <div class="order-card mb-4 shadow-sm border rounded-0" style="max-width: 700px; width: 100%;">
            <div class="order-products bg-white p-4 rounded-0">
                <div class="mb-4 d-flex align-items-start gap-3">
                    <img src="{{ $item->product->images->first() ? asset('storage/' . $item->product->images->first()->image_path) : '/assets/img/products/1.png' }}" alt="" style="width: 80px; height: 80px; object-fit: cover; border: 1px solid #ddd;">
                    <div class="flex-grow-1">
                        <div class="fw-bold">{{ $item->product_name }}</div>
                        <div class="text-muted small mt-1">
                            @if ($item->product_options)
                                @php $opts = json_decode($item->product_options, true); @endphp
                                @if (!empty($opts['sku'])) Mã SP: {{ $opts['sku'] }} | @endif
                ...
                <form action="{{ route('client.orders.review.submit', [$order->id, $item->id]) }}" method="POST" enctype="multipart/form-data" id="review-form">
                    ...
                    <div class="border p-4 rounded-0" style="background: #fff;">
                        ...
                        <div class="mb-3">
                            ...
                            <textarea name="content" id="content" class="form-control rounded-0" rows="4" maxlength="1000" oninput="updateCharCount()" required></textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="images[]" id="images" class="form-control rounded-0" multiple accept="image/*" onchange="previewImages()">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" name="video" id="video" class="form-control rounded-0" accept="video/*" onchange="previewVideo(); checkVideoSize();">
                        ...
                        <div class="mt-4 d-flex gap-2 justify-content-end align-items-center">
                            <a href="{{ route('client.orders.show', $order->id) }}" class="btn btn-secondary rounded-0 py-2 px-4">Trở lại</a>
